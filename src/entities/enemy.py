...
class Enemy(Entity):
    ...
    def update(self, dt: float) -> None:
        ...
        while distance_to_move > 0 and self.state == EnemyState.MOVING:
            ...
            current_x = current_pos[0] + dx * segment_progress
            current_y = current_pos[1] + dy * segment_progress
            ...
            if current_idx >= len(self.path) - 1:
                ...
                current_pos = self.path[current_idx]
                next_pos = self.path[current_idx + 1]
                self.position.x = current_pos[0] + (next_pos[0] - current_pos[0]) * segment_progress
                self.position.y = current_pos[1] + (next_pos[1] - current_pos[1]) * segment_progress
